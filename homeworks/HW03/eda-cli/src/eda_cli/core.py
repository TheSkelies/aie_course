...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col in summary.columns:
        if col.unique == 1 and col.non_null > 0:
            has_constant = True
            break
    flags["has_constant_columns"] = has_constant # имеет хотя бы одно повторение

    all_stats = len(missing_df) * summary.n_rows # количество всех значений
    missing_stats = missing_df["missing_count"].sum() # количество пропущенных значений
    flags["has_many_missing_values"] = (missing_stats / all_stats) > 0.2 # больше 20 процентов пропущенных значений
